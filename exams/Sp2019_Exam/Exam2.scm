...
; TYPE YOUR NAME HERE: NAN CHEN

; TYPE YOUR FULL EMAIL ADDRESS HERE: nchen000@citymail.cuny.edu
...
;constructor
(define (make-poly leading-coeff degree rest-of-poly)
  (list leading-coeff degree rest-of-poly))
                   
;selector for polynomial 
(define (leading-coeff poly)
  (car poly))
...
;;;;; these classifiers make no sense for polynomial representations -- 
...
;filter works by cdring down seq
;basis: seq is null returns seq
;IH: seq works for seq with length n
;IS: seq works for seq with length n+1
; if car of seq passes pred, we keep the car of seq and filter the rest
; else don't keep car and filter the rest
;idea: filter out the plus sign in list then turn filtered p into r

;;;; would be so much better to build yet another simple datatype for polynomials ... try this, and observe how doing so allows you to
;;;; improve your code


(define (poly-to-rep p)
  (turn-r (filter pair? p)))
        
(define (turn-r e)
  (cond ((= (pow e) 0)
         (make-poly (coeff e) (pow e) '()))
        (else (make-poly (coeff e) (pow e) (turn-r (cdr e))))))

;;;; ok, except for the assumption about the stopping case
