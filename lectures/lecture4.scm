...
;My ANSWER
;We will induct on n.
;Basis Step: When n=0, 0 is returned. (I think that I should use 1 to test the basis step instead of
;            0 because 1 is closer to the number range that our program will operat with.)
;Induction Hypothesis: on the k-th call (fib n) returns the n-th fibonacci number when n is an
;                      integer >=0.
;Induction Step: On the k+1th call, (fib(n + 1)) = (fib n) + n which returns the
;                correct value of nth fibonacci number.
;Termination: The program terminates when n reaches 1 and 0, and the summation of each fib occurs
;             the first fib call.
