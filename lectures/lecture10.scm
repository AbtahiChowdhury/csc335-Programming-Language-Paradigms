...
(define (accumulate-left op acc seq)
  (cond ((null? seq) acc)
	(else (accumulate-left op (op (car seq) acc) (cdr seq)))))

; examples ...

(define (elem e l)
  (accumulate-left (lambda (x y) (if (eq? e x) #t y)) #f l))
...
;  do we need to separate out treatment of the car?

(define (count-nodes tree)
  (cond ((null? tree) 0)
        ((atom? tree) 1)
        (else (+ 1 (accumulate + 0 (map count-nodes tree))))))
